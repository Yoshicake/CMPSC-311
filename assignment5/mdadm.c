...
int mdadm_mount(void) {
  ...
  jbod_client_operation(encode(JBOD_MOUNT,0,0), NULL);
  mounted = 1;
  return 1;
}
...
int mdadm_unmount(void) {
  ...
  jbod_client_operation(encode(JBOD_UNMOUNT,0,0), NULL);
  mounted = 0;
  return 1;
}
...
int mdadm_write_permission(void){
  ...
  jbod_client_operation(encode(JBOD_WRITE_PERMISSION,0,0), NULL);
  write_perm = 1;
  return 1;
}
...
int mdadm_revoke_write_permission(void){
  ...
  jbod_client_operation(encode(JBOD_REVOKE_WRITE_PERMISSION,0,0), NULL);
  write_perm = 0;
  return 1;
}
  ...
  while (read_len > 0){
    ...
    // Cache hit: copy data directly to read_buf
      ...
      if (blockID == JBOD_NUM_BLOCKS_PER_DISK) {
        ...
        diskID++;
        if (read_len > 0) {
          jbod_client_operation(encode(JBOD_SEEK_TO_DISK, diskID, 0), NULL);
        }
      }
    ...
    jbod_client_operation(readOp, buffer);
  ...
  uint8_t buffer[JBOD_BLOCK_SIZE];
    ...
    if (cache_insert(diskID, blockID, buffer) == -1){
	  ...
	  uint32_t writeOp = encode(JBOD_WRITE_BLOCK, 0, 0);
   	jbod_client_operation(writeOp, buffer);
